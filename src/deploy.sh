...
echo "   ip route get \${VPN_SERVER_IP}                    # expect via 10.0.0.1 (ISP)"
echo "   ip route get 77.88.8.8                          # expect via 10.0.0.1 (RU -> ISP)"
...
echo "   # After rollback: ip route show default → default via 10.0.0.1"
...
echo "   # expect: 2"
echo "   # Keenetic manual step: Home network -> Segments -> DNS server -> 10.0.0.254"
...
echo "   # expect: route via 10.0.0.1 (KEENETIC_GW) for any IP inside an exception CIDR"
echo ""

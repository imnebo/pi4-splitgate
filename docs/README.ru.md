...
Интернет
  ↓
Роутер (10.0.0.1) — подключение к провайдеру
  ↓ eth0
RPi4 (10.0.0.254) — VPN-шлюз
  ↓
Устройства в LAN (шлюз по умолчанию = 10.0.0.254 через DHCP роутера)

Не-RU → awg0 → AmneziaWG VPN (endpoint: <VPN_SERVER_IP>:<port>)
RU-подсети → eth0 → провайдер напрямую (через 10.0.0.1)
```
...
- после перевода DHCP-шлюза роутера на `10.0.0.254` держите RPi на проводном `eth0`; Wi-Fi в той же LAN отключите, чтобы не получить ARP-конфликт
...
1. `http://10.0.0.1` → Домашняя сеть → Сегменты → По умолчанию → Параметры IP
2. Установите **Адрес шлюза** равным `10.0.0.254` → Сохранить
...
1. Домашняя сеть → Сегменты → По умолчанию → DNS-сервер → установите `10.0.0.254` → Сохранить

**Откат**: очистите поле «Адрес шлюза» в роутере (верните `10.0.0.1`).

...
При наличии этого файла с валидными записями скрипт добавляет параметры запроса `&exclude[cidr4]=CIDR`
к URL загрузки, чтобы вышестоящий сервер исключил эти CIDR из ответа.
Если файл отсутствует или пуст — поведение идентично стандартному (без исключений).

Количество исключённых CIDR логируется: `journalctl -t vpn-routes | grep "Excluding"`.
